...
bool direction = true,scale_flip=true;
...
int main()
{
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		scale_amount += scale_flip ? -0.0001f : 0.0001f;
		if (scale_amount <= 0.0f || scale_amount >= 1.0f) scale_flip = !scale_flip;
		...
		mat4 model(1.0f);
		model = scale(model, vec3(scale_amount, scale_amount, 1.0f));
		//model = rotate(model, rot_angle * toRadians, vec3(0.0f, 0.0f, 1.0f));
